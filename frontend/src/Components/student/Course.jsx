     ...
     {/*  <div className="w-1/4 h-screen">
      ...
      </div> */}
      {
        ...
        <div className="base-container py-[5vh] ">
            ...
            <Searchbox text="Search for the course.." searchQuery={searchQuery} setSearchQuery={setSearchQuery}/>
            <hr className="my-5"/>
            <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
              {data?.map((course) => (
                <div
                  key={course.id}
                  className="bg-green-200 w-[270px] h-[250px] rounded-lg bg-contain group cursor-pointer shadow-lg"  style={{ backgroundImage: `url(${course.img})` }} 
                >
                  {/* <img
                    src={course.img}
                    alt={course.name}
                  ...
                  <div className="group-hover:opacity-80 opacity-0 bg-theme text-center flex-col-center w-full h-full rounded-lg p-5 font-comf">
                    <h1 className="text-primary text-3xl font-merri">{course.name}</h1>
